...
echo1 "$0: 1.a.1. demo rules in FoD:" 1>&2
(docker exec -ti "$fod_container_name" ./inst/helpers/list_rules_db.sh | grep "10.1.10.11/32.*10.2.10.12/32" || true)
echo 1>&2
...
echo1 "$0: 1.b.1. show exabgp current exported rules/routes:" 1>&2
docker exec -ti "$fod_container_name" sh -c '. /opt/venv/bin/activate && exabgpcli show adj-rib out extensive'
echo 1>&2

echo1 "$0: 1.b.2. freertr policy-map and block counters:" 1>&2
...
echo1 "$0: 1.b.3. ping not to be blocked:" 1>&2
docker exec -d -ti host1 ping -c 1 10.2.10.12
docker exec -ti host1 ping -c 7 10.2.10.12
echo 1>&2

echo1 "$0: 1.b.4. freertr policy-map and block counters:" 1>&2
docker exec -ti freertr sh -c '{ echo "show ipv4 bgp 1 flowspec summary"; echo "show ipv4 bgp 1 flowspec database"; echo "show policy-map flowspec CORE ipv4"; echo exit; } | netcat 127.1 2323' | output_with_specific_colormarks "drp=[0-9]"
echo 1>&2
...
echo1 "$0: 2.a.1.a. show exabgp current exported rules/routes (before adding the blocking rule):" 1>&2
...
echo1 "$0: 2.a.1.b. show freertr flowspec status/statistics (before adding the blocking rule):" 1>&2
...
echo1 "$0: 2.a.2. proper adding of blocking rule:" 1>&2
...
echo1 "$0: 2.a.3.a. show exabgp current exported rules/routes (after adding the blocking rule):" 1>&2
docker exec -ti "$fod_container_name" sh -c '. /opt/venv/bin/activate && exabgpcli show adj-rib out extensive' | output_with_specific_colormarks .
echo 1>&2

echo1 "$0: 2.a.3.b. show freertr flowspec status/statistics (after adding the blocking rule):" 1>&2
docker exec -ti freertr sh -c '{ echo "show ipv4 bgp 1 flowspec summary"; echo "show ipv4 bgp 1 flowspec database"; echo "show policy-map flowspec CORE ipv4"; echo exit; } | netcat 127.1 2323' 
echo 1>&2
...
echo1 "$0: 2.b.1. show exabgp current exported rules/routes:" 1>&2
docker exec -ti "$fod_container_name" sh -c '. /opt/venv/bin/activate && exabgpcli show adj-rib out extensive' | output_with_specific_colormarks .
echo 1>&2

echo1 "$0: 2.b.2. show freertr flowpec status/statistics (before ping to be blocked):" 1>&2
docker exec -ti freertr sh -c '{ echo "show ipv4 bgp 1 flowspec summary"; echo "show ipv4 bgp 1 flowspec database"; echo "show policy-map flowspec CORE ipv4"; echo exit; } | netcat 127.1 2323' | output_with_specific_colormarks "drp=[0-9]"
echo 1>&2

echo1 "$0: 2.b.3. ping to block:" 1>&2
docker exec -ti host1 ping -c 7 10.2.10.12 || true
echo 1>&2

echo1 "$0: 2.b.4. show freertr flowspec status/statistics (after ping to be blocked):" 1>&2
docker exec -ti freertr sh -c '{ echo "show ipv4 bgp 1 flowspec summary"; echo "show ipv4 bgp 1 flowspec database"; echo "show policy-map flowspec CORE ipv4"; echo exit; } | netcat 127.1 2323' | output_with_specific_colormarks "drp=[0-9]"
echo 1>&2
